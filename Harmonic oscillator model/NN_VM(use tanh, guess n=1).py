...
# ==========================================
# 시각화 (Ground State vs Excited State)
# ==========================================
x_plot = np.linspace(-1e-9, 1e-9, 1000)
...
psi0_exact_plot = get_normalized_curve(psi_ground, x_plot)
psi1_exact_plot = get_normalized_curve(psi_first_excited_exact, x_plot)
psi1_pred_plot = get_normalized_curve(guessed_psi_1, x_plot)

# 위상(부호) 정렬: x > 0 영역의 피크 방향 통일
if psi1_pred_plot[750] < 0:
    psi1_pred_plot = -psi1_pred_plot

plt.figure(figsize=(10, 6))
x_nm = x_plot * 1e9

plt.plot(x_nm, psi0_exact_plot, label='Ground State $\psi_0(x)$', color='gray', linestyle=':', alpha=0.7)
plt.plot(x_nm, psi1_exact_plot, label='Exact Excited $\psi_1(x)$', color='black', linewidth=2.0, linestyle='--')
plt.plot(x_nm, psi1_pred_plot, label='NN Guessed $\psi_1(x)$', color='crimson', linewidth=1.8, alpha=0.85)

plt.title("Harmonic Oscillator $n=1$ Wavefunction Guess", fontsize=14, pad=12)
plt.xlabel("Position $x$ (nm)", fontsize=12)
plt.ylabel("$\psi(x)$ ($\mathrm{m}^{-1/2}$)", fontsize=12)
plt.grid(True, linestyle=':', alpha=0.6)
plt.legend(fontsize=11, loc='upper right')
