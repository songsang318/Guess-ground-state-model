...
def construct_wavefunction(params, x):
    ...
    psi = np.exp(-a * (x**2)) * (sum([
        S_1 * np.sin(np.pi * x) , S_2 * np.sin(2 * np.pi * x) , S_3 * np.sin(3* np.pi * x) , S_4 * np.sin(4 * np.pi * x) , S_5 * np.sin(5 * np.pi * x) , 
        C_1 * np.cos(np.pi * x) , C_2 * np.cos(2 * np.pi * x) , C_3 * np.cos(3* np.pi * x) , C_4 * np.cos(4 * np.pi * x) , C_5 * np.cos(5 * np.pi * x)]))
